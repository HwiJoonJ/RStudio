...
### 1. 데이터 탐색 (EDA) 및 전처리
```{r eda}
filename <- list.files(pattern='B[0-9]') #B+숫자 형태를 가진 파일명 검색
dat <- list()
...
#Merge capacity data with cycle/time
dat_merge <- Reduce(function(x, y){merge(x, y, by=c('cycle', 'time'))}, dat)
head(dat_merge)

dat_merge <- dat_merge[,c(1,2,grep('capacity', colnames(dat_merge)))] #찾고자하는 문자열의 인덱스 반환
...
#종속변수~독립변수
#계측치~기준치
dat_merge <- aggregate(. ~ cycle, data=dat_merge[,-2], FUN = mean)
head(dat_merge)

#Visualization
par(mfrow=c(1,1))
plot(NULL, xlim=c(range(dat_merge$cycle)), ylim=c(range(dat_merge[,2:4])), xlab='cycle', ylab='capacity')
for(i in 2:length(dat_merge)) {
  points(dat_merge$cycle, dat_merge[,i], col=i, pch=19)
}
legend('topright', legend=names(dat), col=2:4, pch=16)

#Reshape data available ANOVA
library(reshape)
