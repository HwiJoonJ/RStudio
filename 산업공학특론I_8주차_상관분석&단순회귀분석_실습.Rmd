...
dat <- aggregate(. ~cycle, data=dat, FUN = mean)
head(dat)
...
#상관계수 테이블 생성 (선형적 관계를 정량적으로 측정 - 피어슨 상관계수)
#켄달 상관계수
#스피어만 상관계수
...
corrplot(corr_p, col=col(200), method='ellipse', addCoef.col = 'black', type='lower', title="Pearson Correlation Plot")
...
plot(x, y, pch=16)
abline(a = reg$coef[1], b = reg$coef[2], col='red')

# 신뢰구간, 예측구간 산출
new_dat <- 1:max(dat$cycle)

conf <- predict.lm(reg, data.frame(new_dat), interval="confidence")
pred <- predict.lm(reg, data.frame(new_dat), interval="prediction")

plot(x, y, pch=16)
for(i in 2:3) {
  lines(new_dat, conf[,i], lwd=2, lty=2, col=2)
  lines(new_dat, pred[,i], lwd=2, lty=3, col=3)
}
